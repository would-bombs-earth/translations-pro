...
// ── 控制台样式 ──
const _S_KV =
  "background:#d97706;color:#fff;padding:1px 7px;border-radius:3px;font-weight:600";
const _S_KV_ERR =
  "background:#ef4444;color:#fff;padding:1px 7px;border-radius:3px;font-weight:600";
...
export async function kvPutDomains(domains) {
  ...
  if (!res.ok)
    throw new Error("KV PUT HTTP " + res.status + ": " + resText.slice(0, 80));
}
